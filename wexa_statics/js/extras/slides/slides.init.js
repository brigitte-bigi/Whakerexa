 ...
 *  -------------------------------------------------------------------------
 *
 *  Usage (in the HTML page, before </body>):
 *
 *    <!-- With Whakerexa themes: -->
 *    <script src="wexa_statics/js/extras/slides/slides.init.js"
 *            data-themes="wexa:wexa_theme.css,aurora:wexa_theme_aurora.css,highcontrast:wexa_theme_highcontrast.css"
 *            data-default="wexa"
 *            data-themes-path="wexa_statics/css/themes/">
 *    </script>
 *
 *    <!-- Without themes (own CSS or plain wexa.css): -->
 *    <script src="wexa_statics/js/extras/slides/slides.init.js"></script>
 *
 ...
 *    data-themes-path Path to the themes folder, relative to the HTML page.
 *    data-mode        Initial view mode: "presentation" (default), "handout", "memo".
...
(function () {
    ...
    const themesAttr  = (script.dataset.themes     || '').trim();
    const defaultName = (script.dataset.default    || '').trim();
    const themesPath  = (script.dataset.themesPath || '').trim();
    const mode        = (script.dataset.mode       || 'presentation').trim();
    const logoSrc     = (script.dataset.logo       || '').trim();
    const progressOn  = (script.dataset.progress   !== 'false');
    ...
    function registerThemes(ThemeManager) {
        ...
        for (const entry of themesAttr.split(',')) {
            ...
            t.register(parts[0].trim(), themesPath + parts[1].trim());
        }
        if (defaultName) {
